...
import { clear_action_in_progress, set_action_in_progress } from './registration.actions'
...
export const registrationReducer = createReducer(
  ...
  on(clear_action_in_progress, (state, action) => {
    return {
      ...state,
      actions_in_progress: ([...state.actions_in_progress.filter(a => a.action === action.payload)])
    }
  })
)
